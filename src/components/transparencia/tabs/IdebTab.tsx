...
const csvRows = idebHistorico.map((item) => ({
  ano: item.ano,
  anos_iniciais: item.anosIniciais,
  anos_finais: item.anosFinais,
  meta: item.meta,
}))

export function IdebTab() {
  const hasData = idebHistorico.length > 0 && idebEscolas.length > 0
    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-600">
            Evolucao das notas municipais e comparativo por escola.
          </p>
        </div>
        <TransparenciaExportCsvButton fileName="ideb.csv" rows={csvRows} />
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <Card className="border border-slate-200 py-0">
          <CardHeader>
            <CardTitle>Evolucao historica</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="h-72">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={idebHistorico}>
                  <CartesianGrid strokeDasharray="3 3" vertical={false} />
                  <XAxis dataKey="ano" />
                  <YAxis domain={[4, 7]} />
                  <Tooltip />
                  <Line type="monotone" dataKey="anosIniciais" stroke="#0B4F8A" strokeWidth={2} />
                  <Line type="monotone" dataKey="anosFinais" stroke="#0EA5E9" strokeWidth={2} />
        ...
        <Card className="border border-slate-200 py-0">
          ...
          <CardContent>
            <div className="h-72">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={idebEscolas}>
                  <CartesianGrid strokeDasharray="3 3" vertical={false} />
                  <XAxis dataKey="escola" tick={{ fontSize: 11 }} />
                  <YAxis domain={[0, 7]} />
                  <Tooltip />
                  <Bar dataKey="nota" fill="#0B4F8A" radius={[6, 6, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
