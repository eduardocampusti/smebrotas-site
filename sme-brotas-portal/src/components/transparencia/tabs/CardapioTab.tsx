...
export function CardapioTab() {
  if (!cardapioSemanal.length) return <TransparenciaEmptyState />
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div><h3 className="text-xl font-bold text-slate-900">Cardapio</h3><p className="text-sm text-slate-600">Grade semanal de refeicoes da rede municipal.</p></div>
      ...
      <Card className="border border-slate-200 py-0">
        <CardHeader><CardTitle>Tabela semanal (Seg a Sex)</CardTitle></CardHeader>
        <CardContent>
          <Table>
            <TableHeader><TableRow><TableHead>Dia</TableHead><TableHead>Cafe</TableHead><TableHead>Almoco</TableHead><TableHead>Lanche</TableHead></TableRow></TableHeader>
            <TableBody>{cardapioSemanal.map((i) => <TableRow key={i.dia}><TableCell className="font-semibold">{i.dia}</TableCell><TableCell>{i.cafe}</TableCell><TableCell>{i.almoco}</TableCell><TableCell>{i.lanche}</TableCell></TableRow>)}</TableBody>
          </Table>
        </CardContent>
      </Card>
    </div>
