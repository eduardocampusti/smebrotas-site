...
export default function NoticiaEditorPage() {
  ...
  const [initialData, setInitialData] = useState<Partial<Noticia> | undefined>()
  ...
  async function handleSave(formData: Partial<Noticia>) {
    setSaving(true)
    ...
    try {
      ...
      if (!isEditing || formData.slug !== initialData?.slug) {
        ...
        if (existing && existing.id !== id) {
          toast.error('Este link permanente (slug) já está sendo usado por outra notícia.')
          setSaving(false)
          return
        }
      }
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1">
              {isEditing ? 'Atualize as informações da publicação' : 'Crie uma nova publicação para o portal'}
            </p>
          </div>
        </div>
      </div>
